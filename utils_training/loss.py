...
def compute_reward(image1, image2, kp_map1, kp_map2, homographies, nms, distance_threshold=5, compute_metrics=False):
    ...
    image1, image2 = image1.cpu().numpy(), image2.cpu().numpy() # now BxHxW, before Bx1xHxW because images are grayscale
            ...
            if des1.shape[0] > 2 and des2.shape[0] > 2:
                ...
                if tp >= fp:
                    ...
                    mask_batch1[i, kp1_false_positive_matches[:, 1].tolist(),
                                kp1_false_positive_matches[:, 0].tolist(), 0] = 1.0
                else:
    ...
    reward_batch1 = torch.from_numpy(reward_batch1).unsqueeze(1).cuda()
...
def compute_loss(reward, kpmap, mask):
    ...
    loss = loss_matrix / (torch.sum(mask, axis=[1,2,3])+1e-6)
